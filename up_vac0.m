function [ new_AC ] = up_vac0( par_eff,old_AC,k,M,Na )
%更新外部储备集
%par_eff：候选粒子
%k：决策变量维数
%old_AC：外部储备集
...
if ss2 > Na
    %遍历各个目标函数
    for i = 1:M
        lim_f(i,2) = max(new_AC(:,k+i));       %求目标函数i的极大值
        lim_f(i,1) = min(new_AC(:,k+i));        %求目标函数i的极小值
    end
    DD = [];        %拥挤度
    deep = [];
    for i = 1:M
       [val, ind] = sort(new_AC(:,k+i));        %将目标函数i的函数值按照从小到大的顺序排列
